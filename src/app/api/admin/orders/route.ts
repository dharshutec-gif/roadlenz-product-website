...
export async function POST(
  ...
  if (
    !rawItems.length || rawItems.length > 200
  ) {
    return jsonErr(
      400,
  ...
  if (![subtotal, gst, subtotal + gst + shipping].every(Number.isFinite)) return jsonErr(400, "Order amounts exceed the supported range.");
  ...
  mutateDb(
    (next) => {
      ...
      recordAdminActivity(next, auth.session, "create", "orders", order.id, `Created order ${order.orderNumber}.`);
    },
  );
