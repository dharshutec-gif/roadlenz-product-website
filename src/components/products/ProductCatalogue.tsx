...
import cardStyles from "./ProductCatalogue.module.css";
...
export default function ProductCatalogue({
  ...
  const brandNames = useMemo(() => ["All Brands", ...Array.from(new Set(["Hikvision", ...products.map(p=>p.brand?.trim()).filter((x): x is string=>Boolean(x))]))], [products]);
  const [sortMode, setSortMode] = useState<SortMode>("popular");
  const [filterOpen, setFilterOpen] = useState(false);
  const [featuredOnly, setFeaturedOnly] = useState(false);
  const [priceFilter, setPriceFilter] = useState<PriceFilter>("all");
  const [visibleCount, setVisibleCount] = useState(PAGE_SIZE);
  const { cart, change, error: cartError } = useCustomerCart();
  ...
  }, [search, category, brand, sortMode, featuredOnly, priceFilter]);
  ...
  const result = useMemo(() => {
    ...
    const filtered = products.filter((product) => {
      ...
      if (brand !== "All Brands" && product.brand?.toLowerCase() !== brand.toLowerCase()) return false;
  ...
  }, [products, search, category, brand, sortMode, featuredOnly, priceFilter]);
  ...
  function increase(productId: string) { void change(productId, "add", 1); }
  function decrease(productId: string) { const quantity = cart[productId]?.quantity || 0; void change(productId, quantity <= 1 ? "remove" : "set", quantity - 1); }
...
{cartError && <p role="alert" className="fixed bottom-5 left-5 z-50 rounded-lg bg-red-50 p-4 text-red-700">{cartError}</p>}
      {/* =====================================================
          5-SLIDE VIDEO HERO
      ===================================================== */}
      ...
      <section id="product-list" className="scroll-mt-24 py-10 sm:py-12">
        <div className="mx-auto w-[calc(100%-40px)] max-w-[1500px] sm:w-[calc(100%-64px)]">
              ...
              {categoryNames.map((item) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={active}
                    className={`min-h-11 rounded-full border px-5 text-xs font-bold transition ${
                      active
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3"><label className="flex items-center gap-3 text-sm font-semibold text-slate-700">Brand<select aria-label="Brand" className="min-h-11 rounded-full border border-slate-200 bg-white px-5" value={brand} onChange={e=>setBrand(e.target.value)}>{brandNames.map(name=><option key={name}>{name}</option>)}</select></label>{brand !== "All Brands" && <button className="text-sm text-blue-700" onClick={()=>setBrand("All Brands")}>Clear brand</button>}{category === "Accessories" && <p className="text-slate-600">Panic buttons, extension cables, connectors and mounting accessories.</p>}</div>
          ...
          {visibleProducts.length > 0 ? (
            <>
              <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 min-[1600px]:grid-cols-5">
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <SmartImage
                          src={product.images?.find(image=>image.primary)?.src || product.image}
                          alt={product.name}
                          className="h-full w-full object-contain transition duration-500 group-hover:scale-[1.045]"
                          eager={index < 4}
                        />
                      </Link>
                      ...
                      <div className={cardStyles.body}>
                        ...
                        <Link href={`/products/${product.slug}`} className={cardStyles.titleLink}>
                        ...
                        <div className={cardStyles.actions}>
                          <Link
                            href={`/products/${product.slug}`}
                            className="flex min-h-11 items-center justify-center rounded-full border border-[#159dff]/60 bg-white px-3 text-[11px] font-extrabold text-[#087bc2] transition hover:bg-[#eef8ff]"
                          >
                            View Details
                          </Link>
                          ...
                          {quantity === 0 ? (
                            <button
                              ...
                              className="flex min-h-11 items-center justify-center gap-1.5 rounded-full bg-[#159dff] px-3 text-[11px] font-extrabold text-white shadow-[0_8px_18px_rgba(21,157,255,.18)] transition hover:bg-[#087bc2]"
                            >
