...
export const ABOUT_IMAGES = {
  ...
  expertMohamadIrfan: "/media/about/expert-mohamad-irfan.jpg",
  expertVijayaAravind: "/media/about/expert-vijaya-aravind.jpg",
  expertMohana: "/media/about/expert-mohana.jpg",
  expertJijiDharshini: "/media/about/expert-jiji-dharshini.jpg",
  ...
  render() {
    ...
    return (
      <div ref={this.setElement} className={styles.rlOfficeMap} data-map-mode={imageMode ? "image" : "world"}
        ...
        <div className={styles.rlMapBottom}><span>6 locations Â· 4 countries</span><span>Hover or tap a pin</span></div>
        {active && (
          <div id={popupId} key={active.office.id} className={join(styles.rlLocationPopup, active.point.x < 50 ? styles.rlPopupRight : styles.rlPopupLeft)}
            ...
            <p className={styles.rlPopupPlace}>{active.office.city} Â· {active.office.country}</p>
...
export default class AboutExperience extends Component<AboutExperienceProps, AboutState> {
    ...
    return (
      <form className={styles.rlForm} onSubmit={this.submitEnquiry} aria-busy={this.state.submitting} aria-describedby={`${id}-note`}>
        ...
        <button className={join(styles.rlButton, styles.rlButtonBlue, styles.rlSubmit)} type="submit" disabled={this.state.submitting}>
          {this.state.submitting ? "Sendingâ€¦" : this.props.onEnquirySubmit ? "Submit Enquiry" : "Prepare Email Enquiry"}<Icon name="arrow" />
        </button>
  ...
  render() {
    const {
      ...
      contactHref, mapMode = "world", enableAnimations = true,
    } = this.props;
    ...
    return (
      <div ref={this.setRoot} className={join(styles.rlAbout, className)} style={rootStyle} data-about-version="bigfox-text-fixed-v9" data-effects={enableAnimations && !this.state.reducedMotion ? "on" : "paused"}>
        ...
        {/* 1. HERO â€” heading only. No paragraph, CTA, benefit labels or signature. */}
        ...
        {/* 2. WHO WE ARE â€” Bigfox is the company; RoadLenz is its connected-mobility offering. */}
        ...
        {/* 3. OUR PURPOSE â€” independent, image-ready animated cards. */}
        ...
        {/* 4. JOURNEY â€” vertical timeline; supplied 2016â€“2026 history is retained. */}
        ...
        {/* 5. GLOBAL PRESENCE â€” four independent statistic cards, no address list. */}
        ...
        {showLeadership && <section className={join(styles.rlSection, styles.rlLeadershipSection)} aria-labelledby={`${this.prefix}-leaders-title`}>
          <div className={styles.rlContainer}>
            <div className={styles.rlHeadingRow} data-about-reveal>
              <div>
                <span className={styles.rlEyebrow}>Our Experts</span>
                <h2 className={styles.rlTitle} id={`${this.prefix}-leaders-title`}>Meet Our <em>Experts</em></h2>
              </div>
              <p className={styles.rlHeadingNote}>The people behind our software, design and AI solutions.</p>
            </div>
            <div className={styles.rlLeaders}>
              {[
                { name: "MR. IRFAN", role: "Senior Software Developer", image: ABOUT_IMAGES.expertMohamadIrfan },
                { name: "MR. VIJAYA ARAVIND", role: " Digital Marketing Specialist", image: ABOUT_IMAGES.expertVijayaAravind },
                { name: "MS. MOHANA.B", role: "Junior AI Engineer", image: ABOUT_IMAGES.expertMohana },
                { name: "MS. JIJI DHARSHINI.G", role: "Junior AI Engineer", image: ABOUT_IMAGES.expertJijiDharshini },
              ].map((leader, index) => (
                <article className={styles.rlLeader} key={leader.name} data-about-reveal style={{ "--rl-reveal-delay": `${index * 120}ms` } as CSSProperties}>
                  <div className={styles.rlLeaderVisual}>
                    <div className={styles.rlLeaderGlow} aria-hidden="true" />
                    <div className={styles.rlPortrait}>
                      <SafeImage
                        className={styles.rlExpertImage}
                        src={leader.image}
                        alt={leader.name}
                        width={320}
                        height={320}
                  ...
                  <div className={styles.rlLeaderBody}>
                    <h3>{leader.name}</h3>
                    <p className={styles.rlLeaderCompany}>{leader.role}</p>
                  </div>
                </article>
              ))}
            </div>
        ...
        {/* 6. SUPPORT â€” equally sized image cards, real links and accessible buttons. */}
        ...
        {/* 7. CTA â€” content banner, NOT another website footer. */}
