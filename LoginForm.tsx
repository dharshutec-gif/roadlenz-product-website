...
export default function LoginForm({ intent, title, sub, demoEmail, demoPassword, successHref }: {
    ...
    >
      <div className="rounded-3xl border border-line bg-white p-7 shadow-lift sm:p-8">
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          <button type="submit" disabled={state.status === "loading"} className="btn-primary w-full justify-center disabled:opacity-60">
            {state.status === "loading" ? "Signing in…" : "Sign In"} <Icon name="arrowRight" className="h-4 w-4" />
          </button>
        </form>
      </div>

      <div className="mt-4 rounded-2xl border border-dashed border-brand-300 bg-brand-50 p-4">
        <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-brand-700">Demo credentials</p>
