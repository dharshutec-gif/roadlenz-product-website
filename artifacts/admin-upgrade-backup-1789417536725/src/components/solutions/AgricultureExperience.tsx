...
import styles from "./AgricultureExperience.module.css";
...
export function agricultureCtaHref(href: string | undefined, fallback: string) { const path = href?.split('?')[0]; return (path === '/contact' || path === '/request-quote' ? path : fallback) + '?solution=agriculture'; }
...
function FieldRoute({ stages, active, paused }: { stages: SolutionJourneyStage[]; active: number; paused: boolean }) {
  const reduce = useContext(MotionPreference);
  ...
  const segments = useMemo(() => nodes.slice(1).map((end, index) => {
    ...
    const sweepY = Math.max(45, Math.min(275, Math.min(start.y, end.y) - 48));
    return { start, curve: `L ${start.x} ${sweepY + 12} Q ${start.x} ${sweepY} ${start.x + 12} ${sweepY} L ${midX - 12} ${sweepY} Q ${midX} ${sweepY} ${midX} ${sweepY + 12} L ${midX} ${end.y - 12} Q ${midX} ${end.y} ${midX + 12} ${end.y} L ${end.x} ${end.y}` };
  }), [nodes]);
  ...
  return <div className={styles.routeCanvas}>
    <svg viewBox="0 0 600 320" aria-hidden className={styles.routeSvg}>
      <defs><pattern id={patternId} width="9" height="9" patternUnits="userSpaceOnUse" patternTransform="rotate(-18)"><path d="M0 0V9" stroke="#91a26d" strokeWidth="2" opacity=".25" /></pattern></defs>
      <rect width="600" height="320" fill="#172b29" />
      <g stroke="#a5baa2" strokeWidth="1" strokeOpacity=".35">
        <path d="M12 22L156 8L172 98L24 115Z" fill="#3a5140" />
        <path d="M183 12L383 8L389 125L196 130Z" fill="#304a35" />
        <path d="M410 9L586 24L580 123L414 116Z" fill="#46513a" />
        <path d="M18 140L177 120L196 300L24 305Z" fill="#424a33" />
        <path d="M204 149L389 140L400 307L222 301Z" fill="#344c3a" />
        <path d="M416 140L582 141L574 307L419 299Z" fill="#3b5135" />
      </g>
      <rect width="600" height="320" fill={`url(#${patternId})`} />
      <path d="M173 0L211 320M397 0L410 320M0 129L600 132" fill="none" stroke="#9b9a78" strokeWidth="7" opacity=".27" />
      <g fill="#61886a" opacity=".6">{Array.from({ length: 16 }, (_, i) => <circle key={i} cx={15 + i * 37} cy={133 + (i % 2) * 5} r={3 + i % 3} />)}</g>
      ...
      {nodes.length > 0 && <motion.g style={{ x: markerX, y: markerY }} data-tractor-marker>
        <g transform="translate(-21 -18)"><path d="M9 0H25V13H38L43 21H4V12H9Z" fill="#4ea145" stroke="#c1df89" strokeWidth="1.5"/><path d="M12 3H22V13H12Z" fill="#123f42"/><path d="M32 13V7" stroke="#becaaf" strokeWidth="2"/><circle cx="11" cy="22" r="9" fill="#15291f" stroke="#b4bf63" strokeWidth="3"/><circle cx="36" cy="23" r="6" fill="#15291f" stroke="#b4bf63" strokeWidth="2"/></g>
      </motion.g>}
    </svg>
  </div>;
}
...
function Hero({ solution }: { solution: Solution }) {
  ...
  return <section className={styles.hero}>
    ...
    <div className="shell w-full"><motion.div className={styles.heroContent} initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: reduce ? 0 : .09 } } }}>
      ...
      <motion.div variants={reveal} className={styles.actions}><Link className="btn-primary" href={agricultureCtaHref(solution.heroCtaHref, '/request-quote')}>{solution.heroCtaLabel}</Link><Link className={styles.outlineButton} href={agricultureCtaHref(solution.ctaPrimaryHref, '/contact')}>{solution.ctaPrimaryLabel}</Link></motion.div>
    </motion.div><div className={styles.heroStrip}>{!reduce && <motion.i className={styles.telemetrySignal} animate={{ scaleX: [.05, 1], opacity: [0, .7, 0] }} transition={{ duration: 4, repeat: Infinity }} />}{solution.liveStatus?.map((item, i) => <span key={i}><i /><small>{item.label}</small>{item.value}</span>)}{solution.agricultureContent?.demoLabel && <small>{solution.agricultureContent.demoLabel}</small>}</div></div>
  </section>;
}
function FieldOperationsRoom({ solution }: { solution: Solution }) {
  const reduce = useContext(MotionPreference), content = solution.agricultureContent ?? {};
  ...
  return <section id="field-operations-room" className={`${styles.dark} ${styles.control}`} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} onFocusCapture={() => setFocus(true)} onBlurCapture={e => { if (!e.currentTarget.contains(e.relatedTarget as Node | null)) setFocus(false); }} onTouchStart={e => { touch.current = { x: e.touches[0].clientX, y: e.touches[0].clientY }; }} onTouchEnd={e => { const start = touch.current; touch.current = undefined; if (!start || !e.changedTouches.length) return; const dx = e.changedTouches[0].clientX - start.x, dy = e.changedTouches[0].clientY - start.y; if (Math.abs(dx) > 45 && Math.abs(dx) > Math.abs(dy)) choose(active + (dx < 0 ? 1 : -1)); }}>
    <div className="shell"><Heading eyebrow={content.journeyEyebrow} title={content.journeyTitle} summary={content.journeySummary} />
      <div className={styles.controlGrid}><div className={styles.telemetry}><div className={styles.thumbnail}><Media media={{ type: 'image', src: solution.vehicleImage }} alt={solution.name} contain /></div><dl>{stage?.details?.map((item, i) => <div key={i}><dt>{item.label}</dt><dd>{item.value}</dd></div>)}</dl></div>
        <div><FieldRoute stages={stages} active={active} paused={paused} /><div className={styles.stageCopy}>{stages.map((item, i) => <motion.p key={i} aria-hidden={i !== active} initial={false} animate={{ opacity: i === active ? 1 : 0 }} transition={{ duration: reduce ? 0 : .25 }}>{item.description}</motion.p>)}</div></div>
...
function ConnectedTechnology({ solution, devices }: { solution: Solution; devices: Device[] }) {
  const content = solution.agricultureContent ?? {}, root = useRef<HTMLDivElement>(null), vehicle = useRef<HTMLDivElement>(null), cards = useRef<(HTMLDivElement | null)[]>([]);
