...
import styles from "./EmployeeTransportExperience.module.css";
...
export function employeeCtaHref(href: string | undefined, fallback: string) { const path = href?.split('?')[0]; return (path === '/contact' || path === '/request-quote' ? path : fallback) + '?solution=employee-transport'; }
...
function ShiftRoute({ stages, active, paused }: { stages: SolutionJourneyStage[]; active: number; paused: boolean }) {
  const reduce = useContext(MotionPreference);
  ...
  return <div className={styles.routeCanvas}>
    <svg viewBox="0 0 600 320" aria-hidden className={styles.routeSvg}>
      ...
      {nodes.length > 0 && <motion.g style={{ x: markerX, y: markerY }} data-shuttle-marker>
        <g transform="translate(-24 -19)"><path d="M3 0H35Q44 0 48 11V24H0V5Q0 0 3 0Z" fill="#f2f6fa" stroke="#9bc5e0" strokeWidth="1.5"/><path d="M5 4H14V13H5ZM17 4H26V13H17ZM29 4H35L42 13H29Z" fill="#18466b"/><path d="M1 17H47" stroke="#288cdd" strokeWidth="2"/><circle cx="10" cy="25" r="5" fill="#10263d" stroke="#b4c6d2"/><circle cx="38" cy="25" r="5" fill="#10263d" stroke="#b4c6d2"/></g>
      </motion.g>}
    </svg>
  </div>;
}
...
function Hero({ solution }: { solution: Solution }) {
  ...
  return <section className={styles.hero}>
    ...
    <div className="shell w-full"><motion.div className={styles.heroContent} initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: reduce ? 0 : .09 } } }}>
      ...
      <motion.div variants={reveal} className={styles.actions}><Link className="btn-primary" href={employeeCtaHref(solution.heroCtaHref, '/request-quote')}>{solution.heroCtaLabel}</Link><Link className={styles.outlineButton} href={employeeCtaHref(solution.ctaPrimaryHref, '/contact')}>{solution.ctaPrimaryLabel}</Link></motion.div>
    </motion.div><div className={styles.heroStrip}>{!reduce && <motion.i className={styles.telemetrySignal} animate={{ scaleX: [.05, 1], opacity: [0, .7, 0] }} transition={{ duration: 4, repeat: Infinity }} />}{solution.liveStatus?.map((item, i) => <span key={i}><i /><small>{item.label}</small>{item.value}</span>)}{solution.employeeContent?.demoLabel && <small>{solution.employeeContent.demoLabel}</small>}</div></div>
  </section>;
}
function ShiftJourneyRoom({ solution }: { solution: Solution }) {
  const reduce = useContext(MotionPreference), content = solution.employeeContent ?? {};
  ...
  return <section id="shift-journey-room" className={`${styles.dark} ${styles.control}`} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} onFocusCapture={() => setFocus(true)} onBlurCapture={e => { if (!e.currentTarget.contains(e.relatedTarget as Node | null)) setFocus(false); }} onTouchStart={e => { touch.current = { x: e.touches[0].clientX, y: e.touches[0].clientY }; }} onTouchEnd={e => { const start = touch.current; touch.current = undefined; if (!start || !e.changedTouches.length) return; const dx = e.changedTouches[0].clientX - start.x, dy = e.changedTouches[0].clientY - start.y; if (Math.abs(dx) > 45 && Math.abs(dx) > Math.abs(dy)) choose(active + (dx < 0 ? 1 : -1)); }}>
    <div className="shell"><Heading eyebrow={content.journeyEyebrow} title={content.journeyTitle} summary={content.journeySummary} />
      <div className={styles.controlGrid}><div className={styles.telemetry}><div className={styles.thumbnail}><Media media={{ type: 'image', src: solution.vehicleImage }} alt={solution.name} contain /></div><dl>{stage?.details?.map((item, i) => <div key={i}><dt>{item.label}</dt><dd>{item.value}</dd></div>)}</dl></div>
        <div><ShiftRoute stages={stages} active={active} paused={paused} /><div className={styles.stageCopy}>{stages.map((item, i) => <motion.p key={i} aria-hidden={i !== active} initial={false} animate={{ opacity: i === active ? 1 : 0 }} transition={{ duration: reduce ? 0 : .25 }}>{item.description}</motion.p>)}</div></div>
...
function ConnectedTechnology({ solution, devices }: { solution: Solution; devices: Device[] }) {
  const content = solution.employeeContent ?? {}, root = useRef<HTMLDivElement>(null), vehicle = useRef<HTMLDivElement>(null), cards = useRef<(HTMLDivElement | null)[]>([]);
