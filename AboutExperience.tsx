...
/** Approved visual reference, heading-only hero, motion effects and accessible office popovers.
...
export default class AboutExperience extends Component<AboutExperienceProps, AboutState> {
  ...
  render() {
    const {
      ...
      showLeadership = false, showEnquirySection = false, showImpactStats = false, showOfficeLinks = false,
    ...
    return (
      <div ref={this.setRoot} className={join(styles.rlAbout, className)} style={rootStyle} data-about-version="bigfox-support-rebuild-v8" data-effects={enableAnimations && !this.state.reducedMotion ? "on" : "paused"}>
        ...
        <section ref={this.setHero} className={styles.rlHero} data-hero-visible="true" aria-labelledby={`${this.prefix}-hero-title`}>
          ...
          <div className={join(styles.rlContainer, styles.rlHeroInner)}>
            <h1 data-hero-heading aria-label="Driven by Vision" className={styles.rlHeroTitle} id={`${this.prefix}-hero-title`}>
              <span className={styles.rlHeroTitleLine}><span>Driven by</span></span>
              <span className={styles.rlHeroTitleLine}><em>Vision</em></span>
            </h1>
        ...
        {showLeadership && <section className={styles.rlSection} aria-labelledby={`${this.prefix}-leaders-title`}><div className={styles.rlContainer}>
          <span className={styles.rlEyebrow}>Our Leaders</span><h2 className={styles.rlTitle} id={`${this.prefix}-leaders-title`}>Guided by <em>Leadership</em></h2>
          <div className={styles.rlLeaders}>
            {[{ name: "MRS. SARANYA SAMPATH", role: "Managing Director", photo: managingDirectorPhotoSrc }, { name: "MR. SAMPATH KUMAR G", role: "Director", photo: directorPhotoSrc }].map((leader) => <article className={styles.rlLeader} key={leader.role}><div className={styles.rlPortrait}><SafeImage className={styles.rlPortraitImage} src={leader.photo} alt={leader.name}><Icon name="values" /></SafeImage></div><h3>{leader.name}</h3><p>{leader.role}</p></article>)}
          </div>
        </div></section>}
